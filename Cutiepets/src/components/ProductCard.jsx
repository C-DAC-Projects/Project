    ...
    return product && (
        <div onClick={()=> {navigate(`/products/${product.category.toLowerCase()}/${product._id}`) }} className="border rounded-md md:px-4 px-6 py-1 backdrop-blur bg-white/10 md:border border-gray-300/20 min-w-56 max-w-56 w-full">
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <p className="text-gray-700 font-medium text-lg text-white truncate w-full">{product.name}</p>
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div onClick={(e) => { e.stopPropagation()} } className="text-primary">
                        {!cartItems[product._id] ? (
                            <button className="flex items-center justify-center gap-1 bg-primary border border-indigo-300 md:w-[80px] w-[64px] h-[34px] rounded text-white font-medium cursor-pointer" onClick={() => addToCart(product._id)} >
